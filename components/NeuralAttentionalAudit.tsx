...
export const NeuralAttentionalAudit: React.FC = () => {
  ...
  const finalizeAudit = (userDecidedPoison: boolean) => {
    ...
    const currentBri = parseInt(localStorage.getItem('vigil_operator_bri') || '100');
    const delta = isCorrect ? (blindSpots < 5 ? 8 : 2) : -20;
    localStorage.setItem('vigil_operator_bri', Math.max(0, Math.min(100, currentBri + delta)).toString());
  };
            ...
            {gameState === 'IDLE' && (
              <div className="text-center space-y-8 animate-in fade-in duration-1000">
                <div className="relative inline-block">
                   <Eye className="w-24 h-24 text-zinc-900" strokeWidth={1} />
                   <div className="absolute inset-0 bg-cyan-500/5 blur-3xl rounded-full" />
                </div>
                <div className="space-y-2">
                  <h4 className="text-xl font-black text-zinc-800 uppercase tracking-[0.4em]">Depth Sensor Offline</h4>
                  <p className="text-[10px] font-mono text-zinc-900 uppercase tracking-widest italic">NEURAL_IDLE_STATE</p>
                </div>
              </div>
            )}
                  ...
                  <div className="w-full max-w-xl p-8 bg-zinc-950 border border-zinc-900 rounded-[2.5rem] space-y-6">
                     ...
                     <p className="text-zinc-400 text-sm leading-relaxed italic font-medium">
                        {auditResult.result === 'SAFE' 
                          ? "Saccadic Depth: OPTIMAL. You successfully forced biological focus over low-entropy character clusters. Deception neutralized."
