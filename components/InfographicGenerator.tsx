...
type BriefingMode = 'ALERT' | 'BLUEPRINT' | 'INSTITUTIONAL';
  ...
  const downloadImage = async () => {
    ...
    try {
      ...
      const canvas = await window.html2canvas(infographicRef.current, {
        backgroundColor: mode === 'INSTITUTIONAL' ? '#ffffff' : '#020202',
  ...
  const getModeStyles = () => {
    switch (mode) {
      ...
      case 'INSTITUTIONAL': return {
        ...
        label: 'INSTITUTIONAL RESEARCH SUMMARY'
      };
      default: return {
        bg: 'bg-[#020202]',
        accent: 'text-red-600',
        ...
        <div className="flex-1 p-6 bg-zinc-900/30 border border-zinc-800 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="space-y-3">
             <div className="flex items-center gap-2">
               ...
               <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.4em]">Briefing Mode Selection</h4>
             </div>
             <div className="flex items-center gap-2">
                {[
                  { m: 'ALERT', l: 'Alert' },
                  { m: 'BLUEPRINT', l: 'Blueprint' },
                  { m: 'INSTITUTIONAL', l: 'Corp' }
                ].map((item) => (
                  <button
                    key={item.m}
      ...
      <div className="relative overflow-hidden rounded-[2.5rem] border border-zinc-900 bg-zinc-950 group shadow-2xl">
        <div className="w-full aspect-[16/9] overflow-hidden">
          ...
          >
            ...
            {/* Background Texture */}
            <div className={`absolute inset-0 opacity-[0.05] pointer-events-none bg-[linear-gradient(90deg,${mode === 'INSTITUTIONAL' ? 'rgba(0,0,0,0.1)' : 'rgba(255,255,255,0.1)'}_1px,transparent_1px),linear-gradient(${mode === 'INSTITUTIONAL' ? 'rgba(0,0,0,0.1)' : 'rgba(255,255,255,0.1)'}_1px,transparent_1px)] bg-[size:60px_60px]`} />
            ...
            <div className="relative z-10 flex justify-between items-start">
              <div className="space-y-4">
                <div className={`flex items-center gap-3 ${mode === 'INSTITUTIONAL' ? 'text-zinc-400' : 'text-zinc-600'}`}>
              ...
              <div className="text-right space-y-3">
                <div className="flex items-center gap-4 justify-end">
                  <span className={`text-5xl font-black tracking-tighter uppercase italic ${mode === 'INSTITUTIONAL' ? 'text-black' : 'text-white'}`}>Vigil</span>
                  <div className={`${mode === 'INSTITUTIONAL' ? 'bg-blue-600' : 'bg-white'} rounded-xl w-14 h-14 flex items-center justify-center shadow-2xl`}>
                    <div className={`${mode === 'INSTITUTIONAL' ? 'bg-white' : 'bg-black'} w-7 h-7 rotate-45`} />
                  </div>
                </div>
                 ...
                 <div className="w-full text-left space-y-16">
                    ...
                    <div className="flex gap-16 items-center">
                       ...
                       <div className="shrink-0 w-48 h-48 bg-zinc-50 border border-zinc-100 rounded-[2rem] p-8 flex flex-col justify-between">
                          ...
                          <div className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em] leading-relaxed">
            ...
            <div className="relative z-10 flex justify-between items-end border-t border-zinc-900/50 pt-10">
              <div className="space-y-2">
                <div className={`text-[10px] font-black uppercase tracking-[0.5em] ${mode === 'INSTITUTIONAL' ? 'text-zinc-400' : 'text-zinc-600'}`}>
                   OFFICIAL BROADCAST // SOLANA SECURITY STANDARD
                </div>
                <div className={`text-[16px] font-black uppercase tracking-[0.3em] italic ${mode === 'INSTITUTIONAL' ? 'text-black' : 'text-white'}`}>
              ...
              <div className="flex items-center gap-12">
                <div className="text-right space-y-1">
                  <div className={`text-[11px] font-black uppercase tracking-widest italic flex items-center gap-3 justify-end ${mode === 'INSTITUTIONAL' ? 'text-blue-600' : 'text-emerald-500'}`}>
                    <div className={`w-2 h-2 rounded-full animate-pulse shadow-lg ${mode === 'INSTITUTIONAL' ? 'bg-blue-600 shadow-blue-500/50' : 'bg-emerald-500 shadow-emerald-500/50'}`} /> 
                    {mode === 'ALERT' ? 'THREAT_LOG: ACTIVE' : 'SYSTEM_STATUS: OK'}
                  </div>
                  <div className={`text-[11px] font-black uppercase tracking-[0.4em] ${mode === 'INSTITUTIONAL' ? 'text-zinc-300' : 'text-zinc-800'}`}>
