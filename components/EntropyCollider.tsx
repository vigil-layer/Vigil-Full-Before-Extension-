...
export const EntropyCollider: React.FC<{ powerSave?: boolean }> = ({ powerSave }) => {
  ...
  const updateGlobalBri = (delta: number) => {
    const current = parseInt(localStorage.getItem('vigil_operator_bri') || '100');
    localStorage.setItem('vigil_operator_bri', Math.max(0, Math.min(100, current + delta)).toString());
  };
            ...
            <div className="w-full flex-1 relative px-8 py-10 overflow-hidden">
               {gameState === 'IDLE' && (
                 <div className="h-full flex flex-col items-center justify-center text-center space-y-8 animate-in fade-in zoom-in duration-1000">
                    <Fingerprint className="w-24 h-24 text-zinc-800" strokeWidth={1} />
                    <div className="space-y-2">
                       <h4 className="text-xl font-black text-zinc-700 uppercase tracking-[0.4em]">Biological Auth Required</h4>
